...
int parent(int v) { return floor((v - 1) / 2); }
...
public:
  ...
  Heap(vector<pair<double, int>> &heap) : heap(heap) { this->buildHeap(); }
  ...
  int getHeapVertexPosition(int i) { return heapVertexPositions[i]; }
  ...
  void siftUp(int n) {
    ...
    while (n > 0 && heap[parent(n)] > heap[n]) {
      ...
      swap(heapVertexPositions[heap[parent(n)].second],
           heapVertexPositions[heap[n].second]);
      n = parent(n);
    }
  }
  ...
  void minHeapify(int n) {
    ...
    if (l < heap.size() && heap[l].first < heap[n].first)
      smallest = l;
    else
      smallest = n;
    if (r < heap.size() && heap[r].first < heap[smallest].first)
      smallest = r;
    if (smallest != n) {
      swap(heapVertexPositions[heap[n].second],
           heapVertexPositions[heap[smallest].second]);
